<script>
	...
	function worksheetNavigate(cur) {
		console.log(
			`🚀 ~ file: SideNav.svelte ~ line 43 ~ worksheetNavigate ~ cur`,
			cur
		);
		loadWorksheet(cur);
	}
	...
	function info(cur) {
		console.log(
			`🚀 ~ file: SideNav.svelte ~ line 52 ~ info ~ cur`,
			cur.problems
		);
		...
		console.log(
			`🚀 ~ file: SideNav.svelte ~ line 59 ~ info ~ lsSheet`,
			lsSheet.problems
		);
		console.log(
			`🚀 ~ file: SideNav.svelte ~ line 54 ~ info ~ supaSheet`,
			supaSheet.problems
		);
	}
	...
	async function freshDbCallForWorksheets() {
		...
		console.log(
			`🚀 ~ file: LeftMenu.svelte ~ line 108 ~ freshDbCallForWorksheets ~ s`,
			s
		);
...
{!collapsed ? 'w-48' : ''}">
	...
	{!collapsed ? 'opacity-100 w-full translate-x-0' : ''}">
			...
			{#if !$currentUser}
				...
				<div
					class="auth flex flex-col h-full justify-center items-stretch">
					<div class="p-2">
						<UserAuth inOrUp="in" />
					</div>
					<div class="p-2">
